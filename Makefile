...
# Give the name of the directory containing the library source files.
USER_LIBDIR ?=	./lib
USER_LIBS ?= ads1231 errors bottle utils custom_eeprom
...
ALIBDIRS = $(wildcard \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR2)/libraries/%) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR2)/libraries/%/utility) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR2)/libraries/%/src) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR2)/libraries/%/src/$(ARCH)) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR2)/hardware/arduino/avr/libraries/%) \
		$(ARDUINO_LIBS:%=$(ARDUINO_DIR2)/hardware/arduino/avr/libraries/%/utility) \
...
CXXSTANDARD =	-std=gnu++11 -fno-threadsafe-statics
